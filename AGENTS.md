...
参照元からコードやアセットを移植・改変した場合は、ルート `LICENCE` 内の
第三者表示を維持する。ビルドしたJARにも同ファイルを必ず含める。

## このファイルの管理

- `AGENTS.md`は常にプロジェクトの最新状態と一致させる。
- 本書は作業規約と実装仕様を兼ねる。数値、ID、条件、例外、優先順位は残し、作業履歴とregistry/dataから機械的に得られる重複一覧は載せない。
- READMEは利用者向けの短い概要、build入口、license参照に絞り、詳細仕様や機能履歴を重複掲載しない。
...
- 依存JAR、展開物、解析・生成scriptは `.tmp/` に置いてGit管理外にし、対象版の実JAR・公式ソースで仕様を確認する。
